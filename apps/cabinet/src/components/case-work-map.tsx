...
const SLOT_PURPOSES: Record<string, string> = {
  ...
  sfr_pay: "Справка о выплатах СФР за 12 месяцев — только при проверке начислений фондом.",
  bank: "Дополнительный финансовый документ — запрашивается только специалистом по индивидуальной необходимости. Не загружайте без явного запроса.",
  ...
  sfr: "Ответ или решение СФР — помогает понять, какие сведения или документы фонд просит уточнить (или почему исключён период).",
};
      ...
      <section className="panel">
        ...
        {work.next_actions.length === 0 ? (
          <>
            ...
            <p>Мы проверяем комплект документов. Следующее сообщение появится в едином чате в кабинете и MAX.</p>
          </>
        ) : (
