"""Case folder FIO + byte-identical original (local + Yandex Disk)."""
...
def test_format_case_disk_folder_name_strips_path_chars() -> None:
    cid = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
    name = format_case_disk_folder_name(
        "Иванов/Иван\Иванович",
        case_id=cid,
    )
    assert "/" not in name
...
def test_mirror_uses_fio_folder_and_same_bytes(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(cm, "lookup_case_client_full_name", lambda _cid: fio)
    monkeypatch.setattr(
        cm,
        "maybe_export_case_chat_throttled",
        lambda _cid, **kw: {"ok": False, "skipped": True},
    )

    result = cm.mirror_case_document_safe(cid, "scan.pdf", original, doc_type="ils")
    assert result.get("ok") is True
    assert uploaded, "disk upload expected"
    doc_uploads = [u for u in uploaded if "/incoming/" in u["path"]]
    assert doc_uploads, uploaded
    path = doc_uploads[0]["path"]
    assert fio in path
    folder_seg = path.split("/")[2]
    assert cid not in folder_seg
    assert doc_uploads[0]["content"] == original
