...
def nudge_consent_before_invoice(
    *,
    ...
    """Комплект готов: без ПДн → гейт «Начать»; с ПДн → только оферта/оплата."""
    ...
    if repo.has_consent(cid):
        ...
        return {"ok": bool(ok), "nudged": "offer"}
    ...
    return {"ok": bool(ok), "nudged": "consent_start"}
