...
# Канон для подсказок сотруднику в чате MAX (не путать с публичным сайтом).
DOCS_CHANNEL_CANON = (
    "Документы клиент присылает прямо в этот чат MAX "
    "(переписка с ботом / чат с сотрудником) — PDF/JPG/PNG, примем, специалист увидит. "
    "Согласие на ПДн уже получено при переходе в MAX до кнопки «Начать» — "
    "не проси согласие и не напоминай про него в ответах клиенту. "
    "Не предлагай личный кабинет на сайте, оплату через ЛК и не зови "
    "«продолжить в кабинете» / «откройте кабинет». "
...
Документы и согласие (канон):
{DOCS_CHANNEL_CANON}
Если уместно просить файлы — скажи прислать их в этот чат MAX. Без кабинета и без согласия.

Контекст диалога:
- опирайся на историю переписки и последнее сообщение клиента (в т.ч. свободный текст);
- не предлагай уйти из MAX на сайт / в кабинет;
- если реплика не про стаж / ИЛС / пенсию / документы СФР — вежливо верни
  к проверке стажа уточняющим вопросом, не развивай постороннюю тему.
...
def _fallback_replies(salutation: str, *, pipeline_status: str | None) -> list[str]:
    ...
    return [
        (
            f"{hello} {action} — можно прямо в этот чат MAX (PDF/JPG/PNG). "
            "Мы готовим документы и план — подаёте через СФР или Госуслуги вы сами."
        )[:400],
        (
            f"{hello} Напоминаем: {action.lower()}. "
            "Файлы пришлите сюда в MAX — примем, специалист увидит."
        )[:400],
        (
            f"{hello} Удобнее всего прислать файлы PDF/JPG прямо в этот чат. "
            "Напишите, если нужна подсказка, какие документы важны на этом шаге."
        )[:400],
    ]
