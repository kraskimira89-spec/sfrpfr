...
from sfrfr.services.staff_reply_suggest import DOCS_CHANNEL_CANON
from sfrfr.services.staff_work_queue import derive_next_action, derive_waiting_on

_SYSTEM = f"""Ты помощник сотрудника сервиса «Проверка стажа».
Мы готовим документы и план, подаёт клиент, решение принимает только СФР.
Не обещай перерасчёт и сумму. Не проси СНИЛС текстом в чат (сканы файлами — можно).

Документы и каналы (канон):
{DOCS_CHANNEL_CANON}
В текстах для клиента: сначала предложи прислать файлы в этот чат MAX;
дополнительно можно упомянуть личный кабинет на сайте cabinet.proverkastaza.ru.
Запрещено писать «загружайте только в личный кабинет» / «не в этот чат».
Не предлагай уйти из MAX в кабинет «чтобы продолжить переписку».
Если клиент пишет не по теме стажа/ИЛС — в chat_messages мягко верни к проверке стажа.
...
cabinet_howto (как передать файлы: чат MAX и/или кабинет на сайте). Без ПДн и обещаний.
"""
    ...
    return [
        {
            ...
            "text": (
                ...
                "Документы можно прислать прямо в этот чат MAX (PDF/JPG/PNG) — примем. "
                "Также можно загрузить в личный кабинет на сайте cabinet.proverkastaza.ru. "
        ...
        },
        {
            ...
            "text": (
                ...
                "Файлы — в этот чат MAX или в кабинет на сайте."
            )[:500],
        },
        {
            "kind": "cabinet_howto",
            "text": (
                "Как передать документы: 1) пришлите файл прямо в этот чат MAX "
                "(PDF/JPG/PNG) — примем, специалист увидит; "
                "2) либо откройте личный кабинет на сайте cabinet.proverkastaza.ru → "
                "раздел документов → загрузите файл."
            )[:500],
        },
    ]
