...
{DOCS_CHANNEL_CANON}
В текстах для клиента: предложи прислать файлы в этот чат MAX.
Не упоминай личный кабинет, оплату через ЛК и согласие на ПДн
(согласие уже при входе в MAX до «Начать»).
...
cabinet_howto (как передать файлы: только чат MAX, без кабинета). Без ПДн и обещаний.
"""
    ...
    return [
        ...
        },
        {
            ...
            "text": (
                ...
                "Файлы пришлите в этот чат MAX — примем, специалист увидит."
            )[:500],
        },
        {
            "kind": "cabinet_howto",
            "text": (
                "Как передать документы: пришлите файл прямо в этот чат MAX "
                "(PDF/JPG/PNG) — примем, специалист увидит. "
                "Если файл большой — можно несколькими сообщениями."
            )[:500],
        },
    ]
