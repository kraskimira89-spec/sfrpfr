    ...
    *,
    ...
    if callback:
        ...
        if callback.startswith("llmsoft:"):
            ...
            if soft:
                from sfrfr.integrations.max.intake_from_text import try_resolve_from_user_text
                from sfrfr.services.max_bot_invoice import resolve_offer_cta

                soft_intake = get_intake_store().get_active(user_id)
                offer_pl = resolve_offer_cta(soft)
                if offer_pl:
                    callback = offer_pl
                else:
                    intake_pl = try_resolve_from_user_text(
                        user_text=soft, intake=soft_intake
                    )
                    if intake_pl:
                        callback = intake_pl
                    else:
            ...
            if cid:
                ...
                if out and out.get("skipped"):
                    _reply(
                        ...
                        chat_id=chat_id,
                        text=(
                            "Хорошо — можно идти по плану диагностики самостоятельно. "
                            "Если понадобится помощь, нажмите «Позвать специалиста»."
                        ),
                        case_id=cid,
                    )
                    return MaxHandleResult(
                        ok=True, action="offer_docs_skip", reply="skipped"
                    )
                if out and (
                    out.get("offered") or out.get("reminded") or out.get("order")
                ):
                    return MaxHandleResult(
                        ok=True,
                        action=f"offer_{out.get('tariff', 'step')}",
                        reply="offered",
                    )
            callback = ""
