...
_DOC_TYPE_LABELS_RU = {
    ...
    "passport": "Паспорт",
    "bank_statement": "Банковская выписка за 12 месяцев",
    "sfr_payments": "Справка о выплатах СФР",
    "pension_size": "Справка о размере пенсии",
    "archive": "Архивная справка",
    "military": "Военный билет",
    "children": "Свидетельство о рождении",
...
def _drain_case_chat_pipeline() -> None:
    try:
        ...
        logger.debug("case chat bot pipeline skipped: %s", exc)
    try:
    ...
    if kind == "client" and not _is_internal_staff_message(row):
        ...
        if bot_message_row is None:
            try:
                ...
                logger.info(
                    "bot_job queued corr=%s case=%s job=%s",
                    corr,
                    case_id[:8],
                    (job_id or "")[:8],
                )
            except Exception as exc:  # noqa: BLE001
                logger.warning("case chat bot job enqueue skipped: %s", exc)
