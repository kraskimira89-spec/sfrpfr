...
def add_table(document: Document, rows: list[list[str]]) -> None:
    ...
    for values in rows:
        ...
        for cell, value in zip(cells, values, strict=True):
            cell.text = strip_inline_md(value)
...
def main() -> None:
    ...
    for line in lines:
        ...
        if not line:
            continue
        if line.startswith("# "):
            document.add_heading(strip_inline_md(line[2:]), level=0)
        elif line.startswith("## "):
            document.add_heading(strip_inline_md(line[3:]), level=1)
        elif line.startswith("### "):
            document.add_heading(strip_inline_md(line[4:]), level=2)
        elif line.startswith("- "):
            document.add_paragraph(strip_inline_md(line[2:]), style="List Bullet")
        elif ordered_text := ordered_item_text(line):
            document.add_paragraph(strip_inline_md(ordered_text), style="List Number")
