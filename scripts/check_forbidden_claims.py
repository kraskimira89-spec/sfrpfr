...
# 2. Прямые отрицания ("не гарантируем перерасчёт", "не обещаем", "не отправляйте в публичный канал")
...
def main() -> int:
    ...
    # Канон: личный чат MAX + кабинет OK; запрещены «только кабинет» и абсолютный запрет MAX.
    default_channels = [
        "документы только в кабинете",
        "документы — только в кабинете",
        "сканы — только в кабинете",
        "только в личный кабинет",
        "только через личный кабинет",
        "не присылайте документы в чат",
        "не отправляйте документы в max",
        "кабинет в max",
    ]
