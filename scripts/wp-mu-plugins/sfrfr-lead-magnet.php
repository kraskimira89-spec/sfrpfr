...
function sfrfr_lead_magnet_handle(WP_REST_Request $request)
{
    ...
    if ($channel === 'email' && $email !== '') {
        ...
        $userBody = "Здравствуйте" . ($name !== '' ? ', ' . $name : '') . "!\n\n"
            ...
            . "• Чат-бот MAX: https://max.ru/id8905998693_1_bot?startapp\n"
            . "• Канал MAX: https://max.ru/channel_proverkastaza\n"
            . "• Личный чат с ботом: https://max.ru/id8905998693_1_bot\n\n"
            . "После выписки ИЛС ответьте нам: «ИЛС получил(а)» или «Есть расхождение».\n"
            . "Не отправляйте паспорт, СНИЛС, трудовую и выписку ИЛС в публичный канал. "
            . "После согласия — личный чат MAX или кабинет.\n\n"
            . "Решение о пенсии и перерасчёте принимает только СФР. "
            . "Сервис «Проверка стажа»: https://proverkastaza.ru/\n";
        if (!$marketing) {
    ...
    $message = $channel === 'email'
        ? 'Готово. Проверьте почту: в письме перечень документов для анализа и ссылка на PDF.'
        : 'Готово. Скачайте PDF по ссылке ниже или напишите в MAX: «Нужен чек-лист документов» — бот пришлёт перечень для анализа.';
