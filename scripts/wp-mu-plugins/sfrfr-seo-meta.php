...
add_filter('document_title_parts', static function (array $parts): array {
    $map = [
        ...
        'chek-list-dokumentov' => 'Папка пенсионных документов: чек-лист до обращения в СФР',
        'blog' => 'Статьи',
    ];
    if (is_front_page()) {
...
function sfrfr_seo_description(): string
{
    ...
    if (is_singular()) {
        ...
        $pageDescriptions = [
            ...
            'chek-list-dokumentov' => 'Бесплатный чек-лист: собрать документы для проверки стажа, получить выписку ИЛС и отметить расхождения. Без сканов в чат и без обещания перерасчёта.',
