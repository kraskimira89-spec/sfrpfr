...
if errorlevel 1 (
  "%SSH_BIN%" -f -N -o ExitOnForwardFailure=yes -o BatchMode=yes -o StrictHostKeyChecking=accept-new -o ConnectTimeout=15 -i "%SSH_KEY%" -o IdentitiesOnly=yes -L 127.0.0.1:%SSH_LOCAL_PORT%:127.0.0.1:%SSH_REMOTE_PORT% %SSH_USER%@%SSH_HOST%
  if errorlevel 1 (
