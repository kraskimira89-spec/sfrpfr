...
| Канал специалистов | `channel_proverkastaza_specialists` | — | отдельно, **не** ops-токен | сотрудники | внутренний канал; не путать с ops-ботом |
...
- Не путать **канал специалистов** (`channel_proverkastaza_specialists`) с **ops-ботом** (`id8905998693_3_bot` / `MAX_OPS_BOT_TOKEN`).  
