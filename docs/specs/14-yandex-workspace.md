...
3. **Документы дел:** источник истины — Supabase Storage (кабинет);
   на Диск — **best-effort зеркало** `disk:/SFRFR-cases/{case_id}/`
   (папка = **UUID дела**, без ФИО/телефона/СНИЛС):
   - `incoming/` — сканы клиента;
   - `outgoing/` — подготовленные / подписанные заявления;
   - `chat/history.md` — снимок истории чата (канон — `case_messages`);
   - `meta.txt` — только `case_id` и layout.
...
| **Яндекс Диск** | (1) ops-шаблоны `SFRFR-ops`; (2) зеркало дела `SFRFR-cases/{case_id}/{incoming\|outgoing\|chat}` | папка = UUID; без ФИО в пути; primary = Supabase |
