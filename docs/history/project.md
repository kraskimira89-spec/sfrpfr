...
зеркало Диска, кабинет как хранилище, без amoCRM, LABOR_WORD в оферте.
`docs/history/2026-09-09-legal-customer-journey.md`.
...
- Оферта и cookies без изменений.
- История: `docs/history/2026-08-22-pdn-deepseek-yc.md`.

## 2026-08-22 — дашборд сотрудника

Очередь дел вместо голых счётчиков: ответ, дедлайн, документы, SLA.
Поля `next_action` / `waiting_on`. Миграция `20260822180000_cases_next_action.sql`.

## 2026-08-22 — обратная связь CF7

Одна форма Contact Form 7 с темой по странице; ящик — почта + Flamingo.  
Не смешивать с заявкой на главной (amo) и формой партнёров.
...
- Канал MAX — переписка и приём файлов; документы также в защищённом кабинете на сайте (не форма WP / не публичный канал).
