# Яндекс Трекер + Wiki + CRM: зелёный старт (без Notion)
...
| Качество / SLA из admin | **STAZH** | то же · [yandex-tracker-stazh-quality.md](./yandex-tracker-stazh-quality.md) |
...
| CRM-заметки по лиду/сделке | **кабинет сотрудника** | [playbook-staff-cabinet-crm.md](./playbook-staff-cabinet-crm.md); amo — резерв [`docs/AMO/`](../AMO/README.md) |
...
2. Очереди (канон):
   ...
   - **`STAZH`** — качество: https://tracker.yandex.ru/STAZH
   - **`PUB`** — публикации: https://tracker.yandex.ru/PUB
   - **`FUNNEL`** — воронка ops: https://tracker.yandex.ru/FUNNEL  
   Создание PUB/FUNNEL: `python scripts/create_yandex_tracker_queues.py` (клон `issueTypesConfig` с SFRFR).
3. Типы **Task**, **Веха** (workflow quickStartV2PresetWorkflow).
4. Доски **SFRFR**, **PUB**, **FUNNEL** — Open → In Progress → Done (PUB: можно Backlog/Draft/Ready/Published). Owner UI: FUNNEL-4.
...
- Заметки по клиенту/лиду — в **admin / staff cabinet** (не в Трекер).
- amoCRM выключена (`AMOCRM_ENABLED=0`); пакет `docs/AMO/` — архив на случай включения.
...
- Staff CRM: [playbook-staff-cabinet-crm.md](./playbook-staff-cabinet-crm.md)
- amo (резерв): [../AMO/README.md](../AMO/README.md)
