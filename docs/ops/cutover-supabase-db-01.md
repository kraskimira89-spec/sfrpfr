...
**Старый IP (держать 24–72 ч):** `51.250.13.240` (каталог `b1g0mhpm9tr4lrurk1bu` — без доступа, не трогаем)
...
| TCP | 22 | `185.77.216.16/32` | добавлено (refresh IP) |
| TCP | 443 | `0.0.0.0/0` | добавлено |
| TCP | 80 | `0.0.0.0/0` | добавлено |

Postgres 5432/5433/8000 в интернет **не** открыты (правильно).

## Статус cutover (2026-09-25, вечер)

- ✅ DNS A `supabase.proverkastaza.ru` → **`51.250.69.237`** (ns1.reg.ru, 8.8.8.8, DoH Google).
- ✅ HTTPS с публичной сети отвечает (TLS + HTTP **401** без ключа — ожидаемо для Kong/API).
- ✅ Стек на новой ВМ: Kong, DB, Auth, Storage, Caddy; swap 4 GiB.
- ⚠️ Локальный egress агента Cursor (`149.102.241.241`) **не** достучивается до `51.250.*` / app-VPS — TCP timeout на 22/80/443. Это не даунтайм сервиса: проверка с другой сети (WebFetch) проходит.
- ⏳ Старую ВМ `51.250.13.240` **не гасить** ещё 24–72 ч.
- ⏳ Smoke владельца: кабинет OTP / magic link; при необходимости SG `:5433` ← app-VPS.

**Дальше:** держать старую ВМ; при необходимости SSH — с IP из allowlist или через serial/YC-ассистента (см. `tz-yandex-assistant-ssh-ip-refresh.md`).
