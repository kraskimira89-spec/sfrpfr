...
- По умолчанию ссылка в MAX / ЮKassa **не** создаётся сама — сотрудник нажимает «Ссылка» (копия) или «В MAX» (ссылка + QR + кнопка).
- При `MAX_PAY_LINK_AUTO_SEND=1` и наличии `max_user_id` после черновика вызывается тот же контур, что «В MAX» (`issue_and_deliver_pay_link`).
- «Ссылка» / «В MAX» создают **счёт ЮKassa** (`POST /v3/invoices`, доставка `self`): короткая ссылка вида `https://yookassa.ru/my/i/…`. Если счета в магазине нет — fallback на обычный `confirmation_url`.
- QR строится из этой ссылки (`GET /api/public/pay/{order_id}/qr.png`). SMS и email ЮKassa **не** используем.
- Напоминание в MAX тоже с кнопкой «Оплатить» и QR (не только текст).
- Канон доставки: `docs/ops/playbook-pay-link-to-client.md`.
