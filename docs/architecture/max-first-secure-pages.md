...
- документы **предпочтительно** в кабинет на сайте; вложение в чат MAX **принимаем** (не reject).
...
| Приём вложений в MAX + CTA кабинета сайта | ✅ Есть | `handler.py` accept + staff/очередь; предпочтительно кабинет |
