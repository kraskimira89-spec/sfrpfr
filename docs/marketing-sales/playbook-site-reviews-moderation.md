...
После **✅ Одобрить** бот пишет в чат: «Отзыв опубликован. Проверьте на сайте» + кликабельная ссылка **Открыть этот отзыв** (markdown + кнопка) → `https://proverkastaza.ru/otzyvy/?review={uuid}`.
