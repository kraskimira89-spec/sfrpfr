...
Источник истины по делу — FastAPI + Supabase + кабинет сотрудника. amoCRM — опциональный mirror при повторном включении (`AMOCRM_ENABLED=1`).
...
4. **Основная CRM продукта — кабинет сотрудника**; этот ТЗ — резервный контур amo.
