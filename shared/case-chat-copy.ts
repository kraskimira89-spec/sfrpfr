/** Канонические тексты единого чата по делу (кабинет + MAX). */

export const CASE_CHAT_TITLE = "Чат по делу";

export const CASE_CHAT_SUBTITLE =
  "Один чат в кабинете и MAX. Пишите там, где вам удобнее";

export const CASE_CHAT_SYNC_HINT = "История сообщений синхронизируется.";

export const CASE_CHAT_MAX_BUTTON = "Открыть этот чат в MAX ↗";

export const CASE_CHAT_EMPTY = "Сообщений по делу пока нет";

export const CASE_CHAT_EMPTY_HINT =
  "Напишите вопрос здесь или откройте этот же чат в MAX. Ответы и история будут доступны в обоих вариантах.";

export const CASE_CHAT_DOCUMENTS_RULE =
  "Документы (ИЛС, трудовую и др.) можно прислать в чат MAX или загрузить в разделе «Мои документы». Вопросы пишите в этом чате.";

export const CASE_QUESTIONS_INTRO =
  "Используйте единый чат по делу: в кабинете или в MAX — как удобнее.";

export const CASE_ACTIONS_CHAT_HINT =
  "Есть вопрос по документам? Напишите в чат справа или откройте тот же чат в MAX.";

export const CASE_ACTIONS_CHAT_LINK = "Написать в чате кабинета";
